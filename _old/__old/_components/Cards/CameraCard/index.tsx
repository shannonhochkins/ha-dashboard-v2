...
export function CameraCard({
  ...
  const requestData = useCallback(async () => {
    setLoading(true);
    fetch(
      `https://rwdwrtzkr59smlxgb934b72q647a3zr1.ui.nabu.casa/local/${camera}.json?d=${Date.now()}`,
      {},
    )
      .then((r) => r.json())
      .then(($assets: string[]) => {
  ...
  function relatedDates(a: Asset, b: Asset) {
    return (
      (a.date - b.date >= 0 && a.date - b.date <= TIME_SPAN * 1000) ||
      (b.date - a.date >= 0 && b.date - a.date <= TIME_SPAN * 1000)
    );
  }
  const getRelatedFiles = useCallback(
    (thumbs: Asset[], asset: Asset): Asset[] => {
      const relatedThumbs = thumbs.filter((b) => relatedDates(asset, b));
      const outputThumbs = thumbs.filter(
        (t) => !relatedThumbs.map((x) => x.name).includes(t.name),
      );
      const additional = relatedThumbs.flatMap((x) =>
        getRelatedFiles(outputThumbs, x),
      );
  ...
  const getAvailableThumbnails = useCallback((): Asset[][] | null => {
    ...
    if (selectedDayEntries) {
      const allThumbs = selectedDayEntries.filter(
        (a) => a.name.endsWith("jpg") && a.name.match(/[\d]_thumb\.jpg+/),
      );
      const processed: string[] = [];
      const grouped = allThumbs.reduce(
        (groups, a) => {
  ...
  function findRelatedAssets(thumbnail: Asset) {
    ...
    const selectedDayEntries = { ...assets }[key];
    if (selectedDayEntries) {
      const nonThumnails = selectedDayEntries.filter(
        (asset) => !asset.name.includes("_thumb"),
      );
      nonThumnails.sort(
        (a, b) =>
          Math.abs(a.date - thumbnail.date) - Math.abs(b.date - thumbnail.date),
      );
      ...
      <Popup fillHeight={true} open={open} onClose={() => onClose()}>
        <CameraCardContainer>
          <Row>
            ...
            <PaginationIcon
              onClick={() => currentPage > 1 && updatePageNumber(-1)}
              icon="material-symbols:chevron-left"
            />
            <PaginationIcon
              onClick={() => currentPage !== totalPages && updatePageNumber(1)}
          ...
          {loading ? (
            ...
            <CameraCardContent>
              {thumbnails && thumbnails.length === 0 && (
                <Row>
                  <p>No camera data available for this date.</p>
                </Row>
              )}
              {thumbnails &&
                [...thumbnails].reverse().map((groupedThumbnails, index) => {
                  ...
                  return (
                    groupedThumbnails && (
                      <CameraInner>
                        {groupedThumbnails.length > 1 ? (
                          <Carousel animationHandler="fade" showThumbs={false}>
                          ...
                          <>
                            <CameraImage src={groupedThumbnails[0].path} />
                            <DateDisplay>
                              {createdAt(groupedThumbnails[0].date)}
                            </DateDisplay>
                          </>
                        )}
                        <Actions>
                          <a href={snapshot.path} target="_blank">
                            Raw image{" "}
                          ...
                          {$video.path && (
                            <a
                              ...
                              target="_blank"
                            >
                              {downloadingVideo === groupedThumbnails[0].key
                                ? "Downloading..."
                                : "Video"}
                              {downloadingVideo !==
                                groupedThumbnails[0].key && (
