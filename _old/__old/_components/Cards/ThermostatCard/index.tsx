...
import { useDevice } from "src/_hooks";
...
const Fab = styled.button<{
  ...
  border: 2px solid
    ${(props) =>
      props.active
        ? `var(--ha-button-primary-border)`
        : `var(--ha-button-primary-color)`};
  border-radius: 100%;
  cursor: pointer;
  padding: 0;
  display: flex;
  ...
  outline: 0;
  color: ${(props) =>
    props.active ? `${props.activeColor}` : "var(--ha-button-primary-color)"};
`;
...
export const ThermostatCard = () => {
  ...
  const { current_temperature, fan_mode, max_temp, temperature } =
    ac.attributes || {};
  const state = ac.state as HvacMode;
  const on = state !== "off";
  const [colors, setColors] = useState<string[]>(offColors);
  const [internalFanMode, setInternalFanMode] = useState<FanMode>(
    fan_mode as FanMode,
  );
  ...
  const ac60Elapsed = useMemo(
    () => getElapsedTimeInMinutes(ac_60.attributes.last_triggered),
    [ac_60.attributes.last_triggered, getElapsedTimeInMinutes],
  );
  const ac45Elapsed = useMemo(
    () => getElapsedTimeInMinutes(ac_45.attributes.last_triggered),
    [ac_45.attributes.last_triggered, getElapsedTimeInMinutes],
  );
  const ac30Elapsed = useMemo(
    () => getElapsedTimeInMinutes(ac_30.attributes.last_triggered),
    [ac_30.attributes.last_triggered, getElapsedTimeInMinutes],
  );

  const ac60Active = useMemo(
    () => ac60Elapsed <= 60 && ac_60.state === "on",
    [ac60Elapsed, ac_60],
  );
  const ac45Active = useMemo(
    () => ac45Elapsed <= 45 && ac_45.state === "on",
    [ac45Elapsed, ac_45],
  );
  const ac30Active = useMemo(
    () => ac30Elapsed <= 30 && ac_30.state === "on",
    [ac30Elapsed, ac_30],
  );
  ...
  return internalTemperature !== null ? (
    <Container>
      <CurrentTemperature color={currentTempColor} fontSize={size / 5}>
        {current_temperature}°<span>CURRENT</span>
      </CurrentTemperature>
      <ActionsLeft>
        <Fab
          size={size}
      ...
      <Timers>
        <TimerLabel fontSize={size / 40}>
          {on ? "TURN OFF IN" : "TURN ON FOR"}:
        </TimerLabel>
        <Fab
          size={size / 1.1}
          active={ac60Active}
        ...
        >
          {ac60Active ? 60 - ac60Elapsed : 60}
          {ac60Active ? "m left" : " min"}
        </Fab>
        <Fab
          size={size / 1.1}
          active={ac45Active}
        ...
        >
          {ac45Active ? 45 - ac45Elapsed : 45}
          {ac45Active ? "m left" : " min"}
        </Fab>
        <Fab
          size={size / 1.1}
          active={ac30Active}
        ...
        >
          {ac30Active ? 30 - ac30Elapsed : 30}
          {ac30Active ? "m left" : " min"}
        </Fab>
      </Timers>
      <div
        style={{
